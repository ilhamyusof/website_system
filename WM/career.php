							...
							<div class="row">
								<div class="col-lg-4 col-sm-6">
									<div class="service-card">
										...
										<h3 style="text-transform: uppercase;">Rookies & Interns</h3>
										<?php 
								...
								<div class="col-lg-4 col-sm-6">
									<div class="service-card">
										...
										<?php 
										$statement = $conn->prepare("SELECT
				                              (SELECT COUNT(*) FROM job where status = 'Open' AND Category = 'Corporate')
				                              AS cnt");
				                             $statement->execute();
				                             while($row = $statement->fetch(PDO::FETCH_ASSOC))
				                            {
								...
								<div class="col-lg-4 col-sm-6">
									<div class="service-card">
										<i class='bx bx-briefcase main-icon'></i>
										<h3 style="text-transform: uppercase;">IT executives</h3>
										<?php 
										$statement = $conn->prepare("SELECT
				                              (SELECT COUNT(*) FROM job where status = 'Open' AND Category = 'IT')
				                              AS cnt");
				                             $statement->execute();
				                             while($row = $statement->fetch(PDO::FETCH_ASSOC))
				                            {
										...
										?>
										<?php if ($IT != "0") {?>
										<p style="font-weight: bold; color: #94171B;"><?php echo $IT; ?> Position</p>
											<div style="text-align: right;" class="view-more">
												<a href="detail-job4.php" id="submit">View More<i class='bx bx-right-arrow-alt'></i></a>
											</div>
										<?php } else {} }?>
									</div>
								</div>
								<div class="col-lg-4 col-sm-6">
									<div class="service-card">
										<i class='bx bx-briefcase main-icon'></i>
										<h3 style="text-transform: uppercase;">Operation</h3>
										<?php 
										$statement = $conn->prepare("SELECT
				                              (SELECT COUNT(*) FROM job where status = 'Open' AND Category = 'Operation')
				                              AS cnt");
				                             $statement->execute();
				                             while($row = $statement->fetch(PDO::FETCH_ASSOC))
				                            {
								...
								<div class="col-lg-4 col-sm-6">
									<div class="service-card">
										<i class='bx bx-briefcase main-icon'></i>
										<h3 style="text-transform: uppercase;">Creatives </h3>
										<?php 
										$statement = $conn->prepare("SELECT
				                              (SELECT COUNT(*) FROM job where status = 'Open' AND Category = 'Creative')
				                              AS cnt");
				                             $statement->execute();
				                             while($row = $statement->fetch(PDO::FETCH_ASSOC))
				                            {
